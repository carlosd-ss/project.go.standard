...
import (
	...
	"github.com/project.go.standard/project-web/fiber/mongo/crud-dao/pkg/mongo"
...
"errors"
"fmt"
"github.com/jeffotoni/mercuriuscrud/mongo"
...
func Insert(namecollection string, insert mcustomer.CustomerPost) (Uuid string, err error) {

	// criando session e retorno o db do collection
	col, err := mongo.GetMongoCollection(namecollection)
	if err != nil {
	...
	DataNow := time.Now().Format(mongo.LayoutDate)
	HourNow := time.Now().Format(mongo.LayoutHour)
...
func Find(namecollection, Uuid string) (jsonStr string, err error) {
	...
	// para busca
	var collection mcustomer.CustomerPost
