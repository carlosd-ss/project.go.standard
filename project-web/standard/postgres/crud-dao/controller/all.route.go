...
import (
	...
	mw "github.com/jeffotoni/project.go.standard/project-web/standard/postgres/crud-dao/controller/middleware"
	//"github.com/jeffotoni/project.go.standard/project-web/standard/postgres/crud-dao/internal/cors"
)

//Routes
func Routes(s *handler.Server, cfg cf.Config) *GoServerHttp {
	...
	mux.Handle(Endpoint().Ping, mw.Use(stdlib.NewMiddleware(limiter.New(memory.NewStore(), limiter.Rate{Formatted: "",
		Period: 1 * time.Second, Limit: 1})).Handler(http.HandlerFunc(s.Ping)),
		mw.CustomHeaders(),
		mw.Gzip(),
	...
	mux.Handle("/user", mw.Use(stdlib.NewMiddleware(limiter.New(memory.NewStore(), limiter.Rate{Formatted: "",
		Period: 1 * time.Second, Limit: 15})).Handler(http.HandlerFunc(s.UserPost)),
		mw.CustomHeaders(),
		mw.Logger("/user")))
